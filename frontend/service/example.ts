...
import {ExampleServiceClient, ExampleService} from '~/proto/Example_pb_service'
import {GreetRequest, GreetResponse} from '~/proto/Example_pb';
...
metadata.append("Authorization", /*new TextEncoder().encode*/(`Bearer ${accessToken}`))

const client = new ExampleServiceClient(hostname)

// const client = new ExampleServiceClientImpl(new GrpcWebImpl(hostname, {metadata: metadata}))

export async function test() {
  return new Promise<Empty>(async (res, rej) => client.endpoint(new Empty(), await getAuthMetadata(), (err, resp) => {
    if (err) rej(err);
    else res(resp);
  }));
  // return client.endpoint(Empty);
}

export async function testAuth() {
  // return client.endpointAuth(Empty);
  return new Promise<Empty>(async (res, rej) => client.endpointAuth(new Empty(), await getAuthMetadata(), (err, resp) => {
    if (err) rej(err);
    else res(resp);
  }));
}
...
export async function greet(name: string): Promise<string> {
  ...
  return new Promise<string>(async (res, rej) => client.greet(request, await getAuthMetadata(), (err, resp) => {
    if (err) rej(err);
    else res(resp.getGreeting());
  }));
